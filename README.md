...
- **자동 변환 (FP32)**: `torch.export` 및 `to_edge_transform_and_lower` API를 사용하여 FP32 모델을 `.pte` 파일로 자동 변환합니다.
- **델리게이트 지원**: ExecuTorch 백엔드로 `XNNPACK` (CPU 최적화) 또는 `Portable` (범용)을 선택하여 컴파일 및 실행할 수 있습니다.
...
- **(참고)** 현재 `run_bench.py`는 `benchmark_logic.py`와 동기화되지 않았으며, 초기 버전의 로직을 포함하고 있습니다. 최신 기능과 모델 지원은 GUI 버전을 사용하세요.
...
- 벤치마크 결과를 JSON 파일로 저장하는 기능을 제공하며, 파일명에 모델과 델리게이트 정보(`모델명_fp32_델리게이트명.json`)를 자동으로 포함합니다.
...
<img width="977" height="873" alt="image" src="https://github.com/user-attachments/assets/073f9175-a46c-4ba9-89d8-73800bf8e779" />
...
1. **INT8 양자화 미지원**:
   - 초기 버전(`run_bench.py`)에서는 `torch.ao.quantization`을 사용한 INT8 PTQ(Post-Training Quantization) 기능 구현을 시도했습니다.
   - 그러나 `prepare`된 모델을 ExecuTorch의 Export/Compile API (`torch.export`, `to_edge`, `to_edge_transform_and_lower`)로 처리하는 과정에서 지속적인 호환성 오류가 발생했습니다.
   - 결론: 안정적인 벤치마크 기능을 제공하기 위해 현재 버전에서는 **INT8 양자화 기능을 제외**하고 FP32 벤치마크에 집중했습니다.
2. **`gpt2` 모델 Export 실패**:
